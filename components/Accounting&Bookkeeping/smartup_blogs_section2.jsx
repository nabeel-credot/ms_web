...
function News_and_Blogs_section({ current_post }) {
  ...
  useEffect(() => 
  {
    postCategories.current=current_post?.smartupCategories?.edges?.map((cat)=>{return cat?.node?.id})
    getTwoRecentPost()
    // .then(()=>{
  ...
  useEffect(()=>{
    try{
      ...
      let parsed=JSON.stringify(current_post?.smartupCategories?.edges?.map((cat)=>{return cat?.node?.id}))
  ...
  const getTwoRecentPost=async()=>{
    const response =await gqlRequest({
      "operationName": "GetSmartups",
      "query": `query GetSmartups($first:Int,$excludePostId:ID) {
        smartups (first:$first,where:{notIn:[$excludePostId]}){
            edges {
              node {
                id
                title
      ...
      "variables": {first:2,excludePostId:current_post?.id}
    })
    console.log("recent response in page",response)
    if(response.success===true)
    {
      // setFilteredList(response.data.posts)
      console.log("edges in posts",response.data.smartups.edges)
      setSlicedData(response.data.smartups.edges)
      recentPostsCount.current=response.data.smartups.edges.length
    }
  }
          ...
          <div className={styles.postCD}>
            <div className={styles.post_cty}>
              ...
              <span className={styles.categoryListingWrap}>
                {current_post?.smartupCategories?.edges?.map((it,index)=>
                <span key={`${index}-${Date.now()}`}>
                  <Link href={it?.node?.id?`/smartup/?categories=["${it?.node?.id}"]`:`/smartup`}>{it?.node?.name}</Link>
                  </span>
                )}
                </span>
            </div>
        ...
        <div className={styles.news_blog2_R} data-aos="fade-left">
         {SlicedData?.length>0?<div className={styles.news_blog2_R_sub}>
            ...
            <div className={styles.seeAll}>
              <Link href={'/smartup'}>See All</Link>
            </div>
          </div>:null}
